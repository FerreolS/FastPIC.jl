"""
    build_PIC_operators(profiles; Npix, λ, lenslet_width, T=Float64, pad=0)
...
function build_PIC_operators(profiles::AbstractVector{<:Profile}; Npix, λ, lenslet_width, T = Float64, pad::Int = 0)
    ...
    cx = T.(([ p.position[1] for p in profiles] .+ (pad * 2048 / Npix)) ./ paddedinpix .* 2π)
    cy = T.(([ p.position[2] for p in profiles] .+ (pad * 2048 / Npix)) ./ paddedinpix .* 2π)
    points = (cx, cy)
